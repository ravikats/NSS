 ...
 *  com.empay.staging.serviceImpl.BaseIIOutgoingServiceImpl
...
@Service
public class BaseIIOutgoingServiceImpl
implements BaseIIOutgoingService {
    private static final Logger log = LoggerFactory.getLogger(BaseIIOutgoingServiceImpl.class);
    ...
    public List<StringBuilder> getFeeAndTxnData(List<VisaAcqTxnWorkEntity> feeEntity, List<VisaAcqTxnWorkEntity> txnEntity, DecryptResponseVo response, String acquirerBins, Integer fileSequence) {
        try {
            Integer n;
            String arn;
            ...
            TCRZeroVo line = null;
            if (Objects.isNull(txnEntity) && Objects.isNull(feeEntity)) {
                return null;
            }
            ...
            if (Objects.nonNull(txnEntity)) {
                for (VisaAcqTxnWorkEntity entity : txnEntity) {
                    decryptedCardNumber = (String)response.getCardNumbers().get(entity.getEncCardNumber());
                    arn = entity.getArn();
                    if (Objects.nonNull(decryptedCardNumber)) {
                        ...
                        linesList.add(this.generateFooter91(acquirerBins, fileSequence));
                        continue;
                    }
                    this.updateFailedTxn(arn);
                }
            }
            if (Objects.nonNull(feeEntity)) {
                for (VisaAcqTxnWorkEntity entity : feeEntity) {
                    decryptedCardNumber = (String)response.getCardNumbers().get(entity.getEncCardNumber());
                    arn = entity.getArn();
                    if (Objects.nonNull(decryptedCardNumber)) {
                        n = this.txnCount91;
                        this.txnCount91 = this.txnCount91 + 1;
                        this.totalTxnAmt91 = this.totalTxnAmt91 + entity.getTxnAmount();
                        line = this.getFeeCollection(entity, decryptedCardNumber, acquirerBins);
                        if (Objects.nonNull(line)) {
                            linesList.add(new StringBuilder(line.getFeeCollectionformat()));
                            n = this.tcrCount91;
                            this.tcrCount91 = this.tcrCount91 + 1;
                        }
                        if (this.tcrCount91 <= 3250) continue;
            ...
            log.error("error : getFeeAndTxnData() : " + String.valueOf(e));
    ...
    private void updateFailedTxn(String arn) {
        try {
            List visaEntities = this.visaRepo.findByArn(arn);
            if (visaEntities.isEmpty()) {
                log.warn("No visaEntities found for refNumber: {}", (Object)arn);
                return;
            }
            log.info("Updating genStatus for {} visaEntities with refNumber: {}", (Object)visaEntities.size(), (Object)arn);
            visaEntities.forEach(entity -> entity.setGenStatus(Integer.valueOf(7)));
            this.visaRepo.saveAllAndFlush((Iterable)visaEntities);
            log.info("Successfully updated genStatus for all matching visaEntities.");
        }
        catch (Exception e) {
    ...
    private TCRZeroVo getTcr0(VisaAcqTxnWorkEntity entity, String decryptedCardNumber) {
        ...
        try {
            ...
            tcrZeroVo.setCollectionOnlyFlag(" ");
    ...
    private TCRZeroVo getAdditionalData(VisaAcqTxnWorkEntity entity) {
        TCRZeroVo tcrZeroVo = new TCRZeroVo();
        try {
            tcrZeroVo.setTxnCode(Optional.ofNullable(entity.getTxnCode()).orElse("05"));
            if (entity.getBussAppId() != null && !entity.getBussAppId().isEmpty() && "05".equals(entity.getTxnCode())) {
    ...
    private TCRZeroVo getPaymentServiceData(VisaAcqTxnWorkEntity entity) {
        TCRZeroVo tcrZeroVo = new TCRZeroVo();
        try {
            tcrZeroVo.setTxnCode(Optional.ofNullable(entity.getTxnCode()).orElse("05"));
            tcrZeroVo.setTxnCodeQualifier("0");
    ...
    private TCRZeroVo getChipCardTxnData(VisaAcqTxnWorkEntity entity) {
        TCRZeroVo tcrZeroVo = new TCRZeroVo();
        try {
            tcrZeroVo.setTxnCode(Optional.ofNullable(entity.getTxnCode()).orElse("05"));
            tcrZeroVo.setTxnCodeQualifier("0");
            ...
            tcrZeroVo.setFormFactorIndicator(entity.getFormFactorIndicator() == null ? StringUtils.repeat((char)'0', (int)8) : entity.getFormFactorIndicator());
    ...
    private TCRZeroVo getFeeCollection(VisaAcqTxnWorkEntity entity, String decryptedCardNumber, String acqBin) {
        TCRZeroVo tcrZeroVo = new TCRZeroVo();
        try {
            tcrZeroVo.setTransactionCode(entity.getTxnCode());
            tcrZeroVo.setTxnCodeQualifier("0");
            tcrZeroVo.setTxnComponentSeqNum("0");
            tcrZeroVo.setDestinationIdentifier(StringUtils.leftPad((String)decryptedCardNumber, (int)6, (String)" "));
            tcrZeroVo.setSourceIdentifier(StringUtils.leftPad((String)acqBin, (int)6, (String)" "));
            tcrZeroVo.setReasonCode1(StringUtils.repeat((char)'0', (int)4));
            tcrZeroVo.setCountryCode(StringUtils.repeat((char)' ', (int)3));
            tcrZeroVo.setEventDate(LocalDate.now().format(eventDateFormatter));
            tcrZeroVo.setAccountNumber1(StringUtils.leftPad((String)decryptedCardNumber, (int)16, (String)"0"));
            tcrZeroVo.setAccountNumberExtension1(StringUtils.leftPad((String)decryptedCardNumber, (int)3, (String)"0"));
            tcrZeroVo.setDestinationAmount1(StringUtils.repeat((char)'0', (int)12));
            tcrZeroVo.setDestinationCurrencyCode1(StringUtils.repeat((char)' ', (int)3));
            long longSourceAmount1 = BigDecimal.valueOf(Optional.ofNullable(entity.getTxnAmount()).orElse(0.0)).multiply(this.multiplier).setScale(0, RoundingMode.HALF_UP).longValueExact();
            tcrZeroVo.setSourceAmount1(StringUtils.leftPad((String)String.valueOf(longSourceAmount1), (int)12, (String)"0"));
            tcrZeroVo.setSourceCurrencyCode1(StringUtils.leftPad((String)entity.getTxnCurCode(), (int)3, (String)" "));
            tcrZeroVo.setMessageText(StringUtils.rightPad((String)entity.getArn(), (int)70, (String)" "));
            tcrZeroVo.setSettlementFlag1("9");
            tcrZeroVo.setTransactionIdentifier1(StringUtils.leftPad((String)entity.getTxnId(), (int)15, (String)"0"));
            tcrZeroVo.setReserved8(" ");
            tcrZeroVo.setCentralProcessingDate1(StringUtils.repeat((char)'0', (int)4));
            tcrZeroVo.setReimbursementAttribute1("B");
        }
        catch (Exception e) {
            tcrZeroVo = null;
            log.error("Error : getFeeCollection() :", (Throwable)e);
        }
        return tcrZeroVo;
    }
    ...
    private TCRZeroVo getAFTData(VisaAcqTxnWorkEntity entity) {
        TCRZeroVo tcrZeroVo = new TCRZeroVo();
        try {
            tcrZeroVo.setTransactionCode(entity.getTxnCode());
            tcrZeroVo.setTxnCodeQualifier("1");
