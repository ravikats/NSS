 ...
 *  com.empay.staging.serviceImpl.VisaOutgoingServiceImpl
 *  org.slf4j.Logger
...
import com.empay.staging.service.BaseIIOutgoingService;
...
@Service
public class VisaOutgoingServiceImpl
implements VisaOutgoingService {
    private static final Logger log = LoggerFactory.getLogger(VisaOutgoingServiceImpl.class);
    @Autowired
    AcquirerBinsRepo acqBinRepo;
    @Autowired
    Environment env;
    @Autowired
    FileFormatsRepo fileFormatsRepo;
    @Autowired
    OutFileLogRepo outFileRepo;
    @Autowired
    InterfacesRepo interfacesRepo;
    @Autowired
    BusinessDateRepo businessDateRepo;
    @Autowired
    BaseIIOutgoingService baseIIService;
    @Autowired
    VisaAcqTxnWorkRepo visaTxnWorkRepo;
    @Autowired
    CryptAPI cryptApi;
    @Autowired
    OutgoingSummaryRepo summaryRepo;
    @Autowired
    VisaAcqTxnDataRepo visaAcqDataRepo;
    @Autowired
    OutFileLogRepo outFileLogRepo;
    @Autowired
    PosTransactionRepo posTxnRepo;
    @Autowired
    private IOutGoingSummaryService iOutGoingSummaryService;
    DateTimeFormatter formatter = DateTimeFormatter.ofPattern("ddMMyyyy");

    @Transactional
    public String generateVisaOutgoing(Integer insCode, Integer user, Integer formatCode, String insShortName, LocalDateTime startDate, LocalDateTime toDate) {
        ...
        try {
            ...
            if (startDate == null) {
                feeEntity = this.visaTxnWorkRepo.findByInstitutionCodeAndIntCodeAndGenStatusAndTxnCodeInAndPurchaseDateLessThanEqual(insCode, intCode, Integer.valueOf(3), txnCode, toDate);
                txnEntity = this.visaTxnWorkRepo.findByInstitutionCodeAndIntCodeAndGenStatusAndTxnCodeNotInAndPurchaseDateLessThanEqual(insCode, intCode, Integer.valueOf(3), txnCode, toDate);
                data = this.visaTxnWorkRepo.findByInstitutionCodeAndIntCodeAndGenStatusAndPurchaseDateLessThanEqual(insCode, intCode, Integer.valueOf(3), toDate);
            } else {
                feeEntity = this.visaTxnWorkRepo.findByInstitutionCodeAndIntCodeAndGenStatusAndTxnCodeInAndPurchaseDateBetween(insCode, intCode, Integer.valueOf(3), txnCode, startDate, toDate);
                txnEntity = this.visaTxnWorkRepo.findByInstitutionCodeAndIntCodeAndGenStatusAndTxnCodeNotInAndPurchaseDateBetween(insCode, intCode, Integer.valueOf(3), txnCode, startDate, toDate);
                data = this.visaTxnWorkRepo.findByInstitutionCodeAndIntCodeAndGenStatusAndPurchaseDateBetween(insCode, intCode, Integer.valueOf(3), startDate, toDate);
            }
            this.updateVisaAcqWork(insCode, user, intCode, "", Integer.valueOf(3), startDate, toDate);
            this.updateOutFilelog(fileName, fileId, insCode, outgoingLogSerialNumber);
            Set encryptedCardTokens = data.stream().map(VisaAcqTxnWorkEntity::getEncCardNumber).collect(Collectors.toSet());
            ...
            if (!Objects.nonNull(response) || !Objects.nonNull(response.getCardNumbers())) {
                ...
                this.visaTxnWorkRepo.saveAll((Iterable)data);
                this.updateOutFilelog(fileName, null, insCode, outgoingLogSerialNumber);
                return "Outgoing Failed";
            }
            lines = this.baseIIService.getFeeAndTxnData(feeEntity, txnEntity, response, acqBin.getAcquirerBins(), Integer.valueOf(fileSequence));
            ...
            if (Objects.nonNull(fileId) && Objects.nonNull(lines)) {
                ...
                this.updateVisaAcqWork(insCode, user, intCode, fileId, Integer.valueOf(9), startDate, toDate);
                this.posTxnRepo.completeVISAPosStatus(insCode.intValue());
                this.posTxnRepo.flush();
                this.moveWorkToData(insCode, Integer.valueOf(4));
                ...
                this.iOutGoingSummaryService.generateOutgoingSummaryPDF(user.intValue(), insCode.intValue(), intCode.intValue(), outgoingLogSerialNumber.intValue(), intCategory, insShortName, outGoingFileProcEntity, "");
                return "Success";
            }
            return "Outgoing Failed";
        }
    ...
    private void insertIntoOutgoingSummary(Integer user, Integer insCode, Integer intCode, String fileName, Integer outgoingLogSerialNumber) {
        try {
            List txnEntity = this.visaTxnWorkRepo.findByInstitutionCodeAndGenStatus(insCode, Integer.valueOf(9));
            Map<String, Map> groupedData = txnEntity.stream().collect(Collectors.groupingBy(VisaAcqTxnWorkEntity::getTxnCode, Collectors.collectingAndThen(Collectors.toList(), list -> Map.of("count", Double.valueOf(list.size()), "totalTxnAmount", list.stream().mapToDouble(VisaAcqTxnWorkEntity::getTxnAmount).sum(), "totalSchgAmount", list.stream().mapToDouble(VisaAcqTxnWorkEntity::getSchgAmount).sum()))));
    ...
    private void updateVisaAcqWork(Integer institutionCode, Integer userId, Integer intCode, String fileId, Integer currentStatus, LocalDateTime startDate, LocalDateTime toDate) {
        try {
            List visaEntities = new ArrayList();
            visaEntities = startDate == null ? this.visaTxnWorkRepo.findByInstitutionCodeAndIntCodeAndGenStatusAndPurchaseDateLessThanEqual(institutionCode, intCode, currentStatus, toDate) : this.visaTxnWorkRepo.findByInstitutionCodeAndIntCodeAndGenStatusAndPurchaseDateBetween(institutionCode, intCode, currentStatus, startDate, toDate);
            ...
            if (visaEntities != null && !visaEntities.isEmpty()) {
                ...
                this.visaTxnWorkRepo.saveAllAndFlush(visaEntities);
            } else {
    ...
    private void updateVisaEntity(VisaAcqTxnWorkEntity entity, Integer user, Integer genStatus, String fileId) {
    ...
    private void moveWorkToData(Integer insCode, Integer status) {
        try {
            List entities = this.visaTxnWorkRepo.findByInstitutionCodeAndGenStatus(insCode, status);
            ArrayList<VisaAcqTxnDataEntity> visaEntities = new ArrayList<VisaAcqTxnDataEntity>();
            if (Objects.nonNull(entities)) {
                for (VisaAcqTxnWorkEntity loop : entities) {
                    VisaAcqTxnDataEntity entity = new VisaAcqTxnDataEntity();
                    entity.setSerNumber(loop.getSerNumber());
                    entity.setLastUpdated(LocalDateTime.now());
                    entity.setUpdatedUser(loop.getUpdatedUser());
                    entity.setInstitutionCode(loop.getInstitutionCode().intValue());
                ...
                this.visaAcqDataRepo.saveAllAndFlush(visaEntities);
                this.deleteFromVisaAcqWork(insCode);
            }
        }
        catch (Exception e) {
    ...
    @Transactional
    private void deleteFromVisaAcqWork(Integer insCode) {
        try {
            String outFileId = this.outFileLogRepo.findTopBygeneratedStatusOrderByLastUpdatedDateDesc(Integer.valueOf(4)).getFileId();
            List visaAcqWork = this.visaTxnWorkRepo.findByInstitutionCodeAndFileId(insCode, outFileId);
            if (Objects.nonNull(visaAcqWork) && !visaAcqWork.isEmpty()) {
                this.visaTxnWorkRepo.deleteAll((Iterable)visaAcqWork);
            }
        }
        catch (Exception e) {
            log.error("Error : deleteFromVisaAcqWork : ", (Throwable)e);
        }
    }
}
